...
export default function AddLecionaPage() {
  ...
  const ListProfessor = db.professores.map((professores) => ({
    id: professores.id,
    nome: professores.nome,
  }));
  const ListMateria = db.materias.map((materias) => ({
    id: materias.id,
    nome: materias.nome,
  }));
  const ListSala = db.salas.map((salas) => ({
    id: salas.id,
    sala: salas.sala,
  }));
      ...
      <div className="edit_box">
        <form className="form_box" onSubmit={addEntity}>
          ...
          <Combobox
            ...
            dataKey="id"
            onChange={(value) =>
              setLeciona((prev) => ({ ...prev, idProfessor: value.id }))
            }
            value={leciona.idProfessor}
            id={"idProfessor"}
          />
          <Combobox
            data={ListMateria}
            ...
            dataKey="id"
            onChange={(value) =>
              setLeciona((prev) => ({ ...prev, idMateria: value.id }))
            }
            value={leciona.idMateria}
            id={"idMateria"}
          />
          <Combobox
            data={ListSala}
            ...
            dataKey="id"
            onChange={(value) =>
              setLeciona((prev) => ({ ...prev, idSala: value.id }))
            }
            value={leciona.idSala}
            id={"idSala"}
          />
